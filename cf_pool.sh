...
then
  ...
  curl -s "https://api.telegram.org/bot$TG_BOT_TOKEN/sendMessage?chat_id=$TG_CHATID&text=添加一条新的$CFRECORD_TYPE类型解析记录 $CFRECORD_NAME:$line"
else
#删除失联IP
  echo "删除失联IP:$line"
  echo $CFZONE_ID
  ...
  -H "Content-Type: application/json")
  echo $RESPONSE
  curl -s "https://api.telegram.org/bot$TG_BOT_TOKEN/sendMessage?chat_id=$TG_CHATID&text=删除了失联的解析IP:$line"
fi
